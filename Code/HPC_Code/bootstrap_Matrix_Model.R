...
draw_sample = function(x) {
  ...
  draws = spread_draws(mod, b_matrix_cover, ndraws = 1) %>% 
...
run_model = function(dat) {
    ...
    mod = glmer(matrix ~ small_abs_lat + (small_abs_lat|taxa/pid), data = dat,
                  family = "binomial")
...
result = run_model(res)

save(result, file = paste0(gpath, "Results/boot_Mods/model_", seed, ".Rdata"))
